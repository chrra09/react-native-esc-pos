...
public class PrinterService {
    ...
    public void printSample() throws IOException {
        ...
        printDesign(design, null, "UTF-8");
    }

    public void printDesign(String text, String image, String codePage) throws IOException {
        ByteArrayOutputStream baos = generateDesignByteArrayOutputStream(text, image, codePage);
        write(baos.toByteArray());
    }

    public Bitmap readImage(String filePath, ReactApplicationContext reactContext) throws IOException {
//		URL url = new URL(filePath);
//		Bitmap image = ImageIO.read(url);

//        Uri fileUri = Uri.parse(filePath);
        Bitmap image = null;
        BitmapFactory.Options op = new BitmapFactory.Options();
        op.inPreferredConfig = Bitmap.Config.ARGB_8888;
		byte[] decoded = Base64.getDecoder().decode(filePath); 
        image = BitmapFactory.decodeByteArray(decoded, 0, decoded.length, op);
        return image;
    }
     ...
     **/
    private ByteArrayOutputStream generateDesignByteArrayOutputStream(String text, String image, String codePage) throws IOException {
        BufferedReader reader = new BufferedReader(new StringReader(text.trim()));
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        String line;

//        byte[] ESC_AT = new byte[] { 0x1b, 0x40 };
//        baos.write(ESC_AT);
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (image instanceof String) {
                try {
                    imageToWrite = generateImageByteArrayOutputStream(readImage(image, context)).toByteArray();
                } catch (IOException e) {
                    throw new IOException(e);
                }
            }
            ...
            byte[] ESC_t = new byte[] { 0x1b, 't', 0x11 };
            byte[] ESC_t1 = new byte[] { 0x1b, 't', 0x09 };
            byte[] ESC_t2 = new byte[] { 0x1b, 't', 0x06 };
            ...
            byte[] LINE_SPACE_30 = new byte[] { 0x1b, 0x33, 30 };
            ...
            if (codePage.equals("Cp866")) {
            	baos.write(ESC_t1);
            }
            if (codePage.equals("Windows-1251")) {
            	baos.write(ESC_t2);
            }
            ...
            try {
                ...
                if (qtToWrite == null && imageToWrite == null && bcToWrite == null) {
                    // TODO: get rid of GBK default!
                    baos.write(layoutBuilder.createFromDesign(line, charsOnLine).getBytes(codePage));
                }
    ...
    private ByteArrayOutputStream generateImageByteArrayOutputStream(Bitmap image) throws IOException {
        ...
        baos.write(LINE_SPACE_24);
        for (int y = 0; y < image.getHeight() + 48; y += 24) {
...
//        for (int x = 0; x < image.getWidth() * 2; x++) {
	        baos.write(new byte[] { (byte) 0x1b, (byte) 0x53 });
//		}
